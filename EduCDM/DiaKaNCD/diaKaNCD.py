...
class KaNCD(CDM):
    ...
    def train(self, train_set, valid_set, lr=0.002, device='cpu', epoch_n=15):
        ...
        for epoch_i in range(epoch_n):
            ...
            for batch_data in tqdm(train_set, "Epoch %s" % epoch_i):
                batch_count += 1
                user_info, item_info, knowledge_emb,student,evalu, y = batch_data
            ...
            auc, acc,rmse,f1 = self.eval(valid_set, device)
    ...
    def eval(self, test_data, device="cpu"):
        ...
        return roc_auc_score(y_true, y_pred), accuracy_score(y_true, np.array(y_pred) >= 0.5),mean_squared_error(y_true, y_pred),f1_score(y_true, y_pred)
