...
from EduCDM.DiaKaNCD import KaNCD
...
def train(data_name):
    ...
    def transform_graph(parser,model,tokenizer,type,teacher,  batch_size):
        ...
        file_AMR='../../data/' + data_name + '/' + type + 'teachers.pkl'
        if not os.path.exists(file_path_amr):
            if not os.path.exists(file_AMR):
                for teacher_batch in teacher:
                    teachers.append(amr_to_graph(AMR(parser,teacher_batch),model,tokenizer))

            else:
                with open(file_AMR, 'rb') as f:
                    teachers_amr = dill.load(f)
                print(file_AMR)
                for teacher in tqdm(teachers_amr):
                    teachers.append(amr_to_graph(teacher, model, tokenizer))
    ...
    for i in range(5):
        ...
        if data_name=='elion':
            cdm = KaNCD(exer_n=item_n, student_n=user_n, knowledge_n=knowledge_n, mf_type='ncf2', dim=128,KCs=final_tensor,input_dim=4096)
            cdm.train(train_set, train_AMR_set, valid_set, valid_AMR_set, epoch_n=20, device="cuda", lr=0.004)
        else:
            cdm = KaNCD(exer_n=item_n, student_n=user_n, knowledge_n=knowledge_n, mf_type='ncf2', dim=6,KCs=final_tensor,input_dim=4096)
            cdm.train(train_set, train_AMR_set,valid_set,valid_AMR_set, epoch_n=1, device="cuda", lr=0.004)
        ...
        auc, accuracy = cdm.eval(valid_set,valid_AMR_set, device="cuda",output_file=file_name_valid,visual=False)
        auc, accuracy = cdm.eval(test_set,test_AMR_set, device="cuda",output_file=file_name_test,visual=False)
